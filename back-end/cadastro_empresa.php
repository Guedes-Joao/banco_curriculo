...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validação básica de CNPJ (só checa formato, não validade real)
    if (!preg_match("/^\d{14}$/", $cnpj)) {
        echo json_encode(["status" => "erro", "mensagem" => "CNPJ inválido. Use apenas números, totalizando 14 dígitos."]);
        exit;
    }
